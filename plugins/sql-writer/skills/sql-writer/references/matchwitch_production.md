# matchwitch_production

Match Witch game event data. All tables share common event schema (see index.md).
...
### New Users
```sql
SELECT COUNT(DISTINCT uid) FROM matchwitch_production.register WHERE dt = 'YYYY-MM-DD'
```
...
FROM matchwitch_production.stageclose
WHERE dt = 'YYYY-MM-DD'
